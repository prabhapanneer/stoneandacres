...
export class CustomersComponent implements OnInit {
  ...
  projectList: any = {}; currentYear: any; enquiry_list: any = [];
  ...
  onSubmit() {
    ...
    this.customerForm.form_data = { name: this.customerForm.name, email: this.customerForm.email, mobile: this.customerForm.mobile, message: this.customerForm.message,  project: this.customerForm.project, form_type: this.customerForm.type };
    this.customerForm.enquiry_list = true;

    this.storeApi.MAIL(this.customerForm).subscribe((result) => {
      this.customerForm.enquiry_list = false;
      if (result.status) {
  ...
  emailBody(formData) {
    ...
    if (!this.enquiry_list?.length) bodyContent = bodyContent.replace("##project##", formData.project);
      ...
      for (let i = 0; i < this.enquiry_list?.length; i++) {
        console.log("for", this.enquiry_list[i], "date", this.datePipe.transform(this.enquiry_list[i]?.created_on, "dd MMM y "));
